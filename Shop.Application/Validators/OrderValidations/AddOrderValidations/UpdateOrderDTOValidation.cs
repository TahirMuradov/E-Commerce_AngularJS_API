...
namespace Shop.Application.Validators.OrderValidations.AddOrderValidations
{
  public  class UpdateOrderDTOValidation :AbstractValidator<UpdateOrderDTO>
    {
        public UpdateOrderDTOValidation (string langCode)
        {
                var culture = new System.Globalization.CultureInfo(langCode);
                RuleFor(x => x.OrderId)
                    .NotEmpty().WithMessage(_ => ValidatorOptions.Global.LanguageManager.GetString("IdIsRequired", culture))
                    .NotEqual(Guid.Empty).WithMessage(_ => ValidatorOptions.Global.LanguageManager.GetString("IdIsRequired", culture));
